...
{
HX_STACK_PUSH("FlxTileblock::new","flixel/tile/FlxTileblock.hx",26);
{
	HX_STACK_LINE(27)
...
int Empties = __o_Empties.Default(0);
	HX_STACK_PUSH("FlxTileblock::loadTiles","flixel/tile/FlxTileblock.hx",43);
	HX_STACK_THIS(this);
	...
	HX_STACK_ARG(Empties,"Empties");
{
		...
		if (((hx::Mod(this->width,sprite->width) != (int)0))){
			HX_STACK_LINE(60)
			this->set_width((::Std_obj::_int(((Float(this->width) / Float(spriteWidth)) + (int)1)) * spriteWidth));
			HX_STACK_LINE(61)
			regen = true;
		}
		HX_STACK_LINE(64)
		if (((hx::Mod(this->height,sprite->height) != (int)0))){
			HX_STACK_LINE(66)
			this->set_height((::Std_obj::_int(((Float(this->height) / Float(spriteHeight)) + (int)1)) * spriteHeight));
			HX_STACK_LINE(67)
			regen = true;
		}
		HX_STACK_LINE(70)
		if ((regen)){
			HX_STACK_LINE(71)
			this->makeGraphic(::Std_obj::_int(this->width),::Std_obj::_int(this->height),(int)0,true,null());
		}
		else{
			HX_STACK_LINE(75)
		...
		while(((row < heightInTiles))){
			...
			while(((column < widthInTiles))){
				HX_STACK_LINE(94)
				if ((((::Math_obj::random() * total) > Empties))){
					HX_STACK_LINE(96)
