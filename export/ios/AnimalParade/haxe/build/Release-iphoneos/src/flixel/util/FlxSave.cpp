...
bool FlxSave_obj::checkBinding( ){
	...
	if (((this->_sharedObject == null()))){
		HX_STACK_LINE(211)
		::flixel::FlxG_obj::log->warn(HX_CSTRING("You must call FlxSave.bind()\nbefore you can read or write data."));
		HX_STACK_LINE(212)
		return false;
	}
	HX_STACK_LINE(214)
...
bool FlxSave_obj::onDone( int Result){
	...
	if (  ( _switch_1==::flixel::util::FlxSave_obj::PENDING)){
		HX_STACK_LINE(186)
		::flixel::FlxG_obj::log->warn(HX_CSTRING("FlxSave is requesting extra storage space."));
	}
	else if (  ( _switch_1==::flixel::util::FlxSave_obj::ERROR)){
		HX_STACK_LINE(188)
		::flixel::FlxG_obj::log->error(HX_CSTRING("There was a problem flushing\nthe shared object data from FlxSave."));
	}
	HX_STACK_LINE(192)
...
bool FlxSave_obj::bind( ::String Name){
	...
	try{
		...
		if (__e.IsClass< ::flash::errors::Error >() ){
			...
			::flash::errors::Error e = __e;{
				HX_STACK_LINE(82)
				::flixel::FlxG_obj::log->error(HX_CSTRING("There was a problem binding to\nthe shared object data from FlxSave."));
				HX_STACK_LINE(83)
				this->destroy();
				HX_STACK_LINE(84)
				return false;
